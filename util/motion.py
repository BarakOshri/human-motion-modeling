...
def load_data(filename, shared=True):
    ...
    batchdata, seqlen, data_mean, data_std, offsets = preprocess_data(Motion)

    # put data into shared memory
    if shared:
        x = theano.shared(np.asarray(batchdata, dtype=theano.config.floatX))
    else:
        x = np.asarray(batchdata, dtype=theano.config.floatX)

    return x, seqlen, data_mean, data_std, offsets
