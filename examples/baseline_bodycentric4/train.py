...
# load data
################################################################################
train_sub_activity_list = ['moving']

tic = clock()

subject = read_subject(path_data)
subject_get_data(subject)

data = subject['taking_food']['activities'][510144139]['data']
data_mean = data.mean()
data_std = data.std()

subject_data_normalize(subject, data_mean, data_std)
...
for activity_label in activity_labels:
    ...
    for id in activities.keys():
        ...
        for sub_activity in sub_activities:
            ...
            if sub_activity_id in train_sub_activity_list:
                S += numpy.sum((data[start_frame:end_frame+1-1, :] -
                                 data[start_frame+1:end_frame+1, :]) ** 2)
    ...
    for activity_label in activity_labels:
        ...
        for id in activities.keys():
            ...
            for sub_activity in sub_activities:
                ...
                if sub_activity_id in train_sub_activity_list:
                    cost_train +=\
                        model.train(data[start_frame:end_frame+1-1, :],
                                    data[end_frame, :],
                                    data[start_frame+1:end_frame+1, :], 
                                    lr)
                    num_seq += 1
