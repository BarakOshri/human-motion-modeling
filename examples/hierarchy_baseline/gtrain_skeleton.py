...
data_joi = np.load(os.path.join(path_dataset, 'data_joi.npy'))
...
model = GRNNL1(dim_g, dim_x, dim_h, dim_y)
...
guide_gen = data_joi[1+start:1+start+n_seed+n_gen, :]
guide = data_joi[start+1:end, :]
...
np.save(os.path.join(path_outputs, 'pos_ggen'), pos_gen)
