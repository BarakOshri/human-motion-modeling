...
model = GRNNL1(dim_x, dim_h, dim_y)
...
while epoch <= 100:
    ...
    for i in range(_index_train.shape[0]): 
        ...
        dout = data[start+1:end, :]
        goal = data[end-1, :]
        loss_train += model.train(goal, din, dout, lr)
...
n_seed = 10
n_gen = 10
goal = data[n_seed+n_gen-1, :]
y_gen= model.generate(goal, data[0:n_seed, :], n_gen)
print 'seed:'
print data[0:n_seed, :]
print 'ground truch:'
