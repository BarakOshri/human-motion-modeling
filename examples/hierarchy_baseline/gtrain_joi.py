...
# Preprocess
num_seq_train = 50 # number of sequences in the training set 
...
model = GRNNL1(dim_x, dim_x, dim_h, dim_y)

toc = clock()
logging.info('Done in %f sec.', toc-tic)

# Training 
def predict_seq(index, i):
    """
    Predict a sequence.
    """
    start = index[i, 0]
    end = index[i, 1]
    guide = np.tile(data[end-1, :], (end-start-1, 1))
    din = data[start:end-1, :]
    return model.predict(guide, din)
...
while epoch <= 2:
    ...
    for i in range(_index_train.shape[0]): 
        ...
        dout = data[start+1:end, :]
        guide = np.tile(data[end-1, :], (end-start-1, 1))
        loss_train += model.train(guide, din, dout, lr)
    ...
    # Test
    if (en_test):
        preds_train = np.concatenate([predict_seq(index_train, i) 
                                for i in range(index_train.shape[0])], axis=0)
        preds_test = np.concatenate([predict_seq(index_test, i) 
                                for i in range(index_test.shape[0])], axis=0)
...
guide_gen = np.tile(data[end-1, :], (n_seed+n_gen, 1))

print 'ground truth:'
print data[start+n_seed:start+n_seed+n_gen, :]

y_gen = model.generate(guide_gen, data[start:start+n_seed])
print y_gen
...
# for T in range(1, 10):
#     guide_T = np.tile(data[end-1, :], (data.shape[0]+T, 1))
# 
#     y_T = model.predict_T(guide_T, data[start:end, :], T)
#     print y_T.shape
...
# n_seed = 10
# n_gen = 10
# g_seed = np.tile(data[n_seed+n_gen-1, :], (n_seed, 1))
# g_gen = np.tile(data[n_seed+n_gen-1, :], (n_gen-1, 1))
# y_gen= model.generate(g_seed, data[0:n_seed, :], g_gen, n_gen)
# print 'seed:'
# print data[0:n_seed, :]
# print 'ground truch:'
