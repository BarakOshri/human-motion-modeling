...
class RNN1L(BaseNN):
    ...
    def __init__(self, 
        ...
        initial_y = T.zeros(cells.layers[-1].hidden_size)
        ...
        for i in range(len(self.cells.layers) - 1):
            layer = self.cells.layers[i]
            if hasattr(layer, 'initial_hidden_state'):
                outputs_info.append(dict(initial=layer.initial_hidden_state, taps=[-1]))
            else:
                outputs_info.append(None)

        # print cells.layers
        # print len(cells.layers)
        # print outputs_info
        # print len(outputs_info)

        results, _ = theano.scan(
            fn=lambda x_t, *states_tm1: _step(x_t, list(states_tm1[1:])),
            sequences=self.x, 
